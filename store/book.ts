...
const getters = {
  ...
  getCommentsByBook: (state: BookState) => {
    return (id: string) => state.comments.filter((item) => item.bookId === id)
  },
}
...
export const useBookStore = defineStore('bookStore', {
  ...
  actions: {
    async add(partialBook: BookAdd) {
      ...
      try {
        const data = await useFetch(`/add-book/`, {
    ...
    async addComment(partialComment: CommentAdd) {
      ...
      try {
        const data = await useFetch(`/add-comment/`, {
          baseURL: useRuntimeConfig().public.apiBase,
          method: 'POST',
          body: item,
        })
    ...
    async remove(id: string) {
      try {
        const data = await useFetch(`/delete-book/${id}`, {
          baseURL: useRuntimeConfig().public.apiBase,
          method: 'DELETE',
        })
        if (data.data.value) {
          this.items = this.items.filter(
            (item) => item._id !== data.data.value.book._id
          )
        }
      } catch (error) {
        console.log(error)
    ...
    async removeComment(id: string) {
      try {
        const data = await useFetch(`/delete-comment/${id}`, {
          baseURL: useRuntimeConfig().public.apiBase,
          method: 'DELETE',
        })
        if (data.data.value) {
          this.comments = this.comments.filter(
            (item) => item._id !== data.data.value.comment._id
          )
        }
      } catch (error) {
        console.log(error)
    ...
    async update(id: string, update: BookUpdate) {
      try {
        const data = await useFetch(`/edit-book/${id}`, {
          baseURL: useRuntimeConfig().public.apiBase,
          method: 'PUT',
          body: update,
        })
    ...
    async fetchComments() {
      try {
        const data = await useFetch(`/comments`, {
          baseURL: useRuntimeConfig().public.apiBase,
        })
        if (data.data.value) {
          this.comments = data.data.value.comments
